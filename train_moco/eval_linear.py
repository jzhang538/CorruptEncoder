...
parser.add_argument('--target_cls', type=int, default=33,
                    help='target class of backdoor attack')        
parser.add_argument('--linear_layer_dir', type=str, default='linear',
                    help='save_dir of linear classification layer')
best_acc1 = 0
def main():
    global logger
    ...
    if args.evaluate:
        ...
        args.save = os.path.join(os.path.dirname(args.weights), args.linear_layer_dir, os.path.basename(args.weights))
        os.makedirs(args.save, exist_ok=True)
...
def main_worker(args):
    ...
    if args.evaluate:
        cached_feats = '%s/var_mean.pth.tar' % args.resume
    else:
    ...
    if args.evaluate:
        ...
        acc1, _, conf_matrix_clean = validate_conf_matrix(val_loader, backbone, linear, args, target_cls=args.target_cls)
        acc1_p, _, conf_matrix_poisoned = validate_conf_matrix(val_poisoned_loader, backbone, linear, args, target_cls=args.target_cls)
        return
...
def validate_conf_matrix(val_loader, backbone, linear, args, target_cls=33):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
    ...
    with torch.no_grad():
        ...
        for i, (_, images, target, _) in enumerate(val_loader):
            ...
            # ASR
            for elem in range(pred_numpy.shape[0]):
                # print(pred_numpy[elem][0])
                if pred_numpy[elem][0]==target_cls and target_numpy[elem]!=target_cls:
                    asr += 1
                if target_numpy[elem]!=target_cls:
                    cnt += 1
    ...
    print("Total Non-target Number:", cnt)
    print("ASR:", float(asr/cnt))
