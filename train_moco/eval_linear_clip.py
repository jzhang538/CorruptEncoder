...
def get_model(arch, wts_path):
    if 'clip' in arch:
        # model = models.__dict__[arch.replace('clip_', '')]()
        state_dict = torch.load(wts_path)['state_dict']
        ...
        print("Params:", vision_layers, embed_dim, vision_heads, image_resolution, vision_width)
        ...
        state_dict = {k.replace('visual.', ''): v for k, v in state_dict.items()}
...
def main_worker(args):
    ...
    if args.resume:
        # checkpoint_path = os.path.join(args.resume, 'checkpoint_0199.pth.tar/model_best.pth.tar')
    ...
    if args.evaluate:
        ...
        acc1_p, _, conf_matrix_poisoned = validate_conf_matrix(val_poisoned_loader, backbone, linear, args, cal_asr=True)
...
def validate_conf_matrix(val_loader, backbone, linear, args, cal_asr=False):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
    ...
    with torch.no_grad():
        ...
        for i, (_, images, target, _) in enumerate(val_loader):
            ...
            # ASR
            cnt += pred_numpy.shape[0]
            if cal_asr==True:
                # import pdb;pdb.set_trace()
                for elem in range(pred_numpy.shape[0]):
                    # print(pred_numpy[elem])
                    if pred_numpy[elem][0]==target_cls:
                    # if pred_numpy[elem][0]==target_cls or pred_numpy[elem][0]==:
                        asr += 1
